...
3. 浏览器打开：
   ...
   http://<LAN-IP>:48080/dashboard.html
   ```
   TurboDrop 默认监听 `0.0.0.0:48080`，同一局域网设备可直接使用启动日志中的 LAN URL 访问控制台。
...
- `Web 主机地址` 默认是 `0.0.0.0`，表示开放给本机与局域网访问；改成 `localhost` 可收回为仅本机访问。
